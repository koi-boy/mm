...
class BIFPN(nn.Module):
    def __init__(self,
        ...
        for ii in range(stack):
            self.stack_bifpn_convs.append(BiFPNModule(channels=out_channels,
                                                      levels=self.bifpn_input_levels - self.start_level,
                                                      conv_cfg=conv_cfg,
                                                      norm_cfg=norm_cfg,
                                                      activation=self.activation))
    ...
    def forward(self, inputs):
        ...
        # add extra feature maps before bifpn
        if self.add_extra_convs_before_bifpn:
            orig = inputs[self.backbone_end_level - 1]
            laterals.append(self.fpn_convs[0](orig))
        ...
        outs = laterals
        # part 2: add extra levels in original ways
        if not self.add_extra_convs_before_bifpn:
...
class BiFPNModule(nn.Module):
    def __init__(self,
        ...
        self.relu2 = nn.ReLU()
        for _ in range(2):  # total 8 depthwise separable convs when level=5
    ...
    def forward(self, inputs):
        ...
        # build top-down
        for i in range(levels - 1, 0, -1):  # i = 4, 3, 2, 1 when levels = 5
        ...
        # build down-top
        for i in range(0, levels - 2, 1):  # i = 0, 1, 2
